...
static __noinline int build(struct __sk_buff *skb,struct ff_request *r,struct ff_interface *iface) {
    ...
    /* Pick a pre-rendered variant per packet: the randomised identity fields
     * then differ between injected datagrams instead of staying fixed for the
     * whole process lifetime. One build feeds every clone, so the duplicates
     * produced by `repeat` keep the same identity and still read as a
     * retransmission of one request. */
    __u32 variant=(bpf_get_prandom_u32()>>26)&(FF_TEMPLATE_VARIANTS-1);
    __u32 tk=r->config_gen*(2*FF_TEMPLATE_VARIANTS)+
        (p.key.protocol==17)*FF_TEMPLATE_VARIANTS+variant;
