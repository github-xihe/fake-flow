...
自动刷新，也可手动刷新。**页面不可见时（切到后台标签页）暂停这个轮询，回到前台立刻
刷新一次** —— 一次轮询要在路由器上起约十个进程（两次 fakeflow、两次 tail、三次 uci、
ubus 与 jsonfilter），隐藏时不值得付这个开销；这也是 LuCI 自带的 poll 不会做的事（它
不看 `document.hidden`）。启动失败仍能从 procd 自己的实例消息在系统日志看到。
