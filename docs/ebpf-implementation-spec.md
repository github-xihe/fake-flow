...
初始验证目标为 Linux 6.6 系列内核、x86_64 和 arm64。OpenWrt 的内核版本、BPF/TC 配置、BTF 可用性需要实机探测；不能只按发行版名称判断。第一版采用 C + libbpf、TC direct-action；TCX 不作为必要依赖。
...
原包可能带大载荷、非线性 skb、GSO/GRO。初版对 GSO/GRO 聚合包和 IP 分片跳过注入，不遍历其中的虚拟报文来冒充精确计数；通过计数器显式报告覆盖缺口。
...
第一版只处理 IPv4 IHL=5、未分片包，以及无扩展头的 IPv6 TCP/UDP；其他布局原样放行并统计。P3 可单独扩展有界 IPv6 扩展头解析，Fragment/AH/ESP 不进入基础注入路径。
